...
echo "second step completed. Time to wait for 300 seconds" >> progress_logs.txt
date >> progress_logs.txt
sleep 9

echo "Finding auth servers now 2nd time" >> progress_logs.txt
...
echo "Third step completed. Time to wait for 300 seconds" >> progress_logs.txt
date >> progress_logs.txt
sleep 9

echo "finding auth servers now 3rd time" >> progress_logs.txt
python3 ./step3_resolve_again_get_ns/get_nameservers.py
...
echo "CONGRATSSSSSSSSSSS!!! its done!">> progress_logs.txt
date >> progress_logs.txt
...
echo "Its time to start step 2" >> progress_logs.txt

cd ../DNS_pipeline_step_2/
bash ronaldo.sh 
echo "Its time to start step 3" >> ../DNS_pipeline_step_1/progress_logs.txt

...
#define SCAMPER_DO_TRACE_GAPLIMIT_DEF  3
...
#define TRACE_OPT_HTTP		  5
#define TRACE_OPT_LOOPS       6
...
const char *scamper_do_trace_usage(void)
{
  return
    ...
    "      [-q attempts] [-s sport] [-S srcaddr] [-t tos] [-U userid] [-F Payload-Filename]\n"
    "      [-w wait-timeout] [-W wait-probe] [-z gss-entry] [-Z lss-name] [-H domain]";
}
 ...
 */
static void trace_hopins(scamper_trace_hop_t **hops, scamper_trace_hop_t *hop, scamper_trace_t *trace)
{
...
static scamper_trace_hop_t *trace_hop(const trace_probe_t *probe,
				      const int af, const void *addr, scamper_trace_t *trace)
{
  ...
  if (probe->ttl == SCAMPER_DO_TRACE_HTTP_TTL_DEF)
    hop->hop_probe_ttl  = trace->tp_ttl;
  else
					   ...
					   scamper_icmp_resp_t *ir)
{
  ...
  if((hop = trace_hop(probe, ir->ir_af, addr.addr, trace)) == NULL)
    goto err;
...
static scamper_trace_hop_t *trace_dl_hop(trace_probe_t *pr,scamper_dl_rec_t *dl, scamper_trace_t *trace)
{
  scamper_trace_hop_t *hop = NULL;

  /* create a generic hop record without any special bits filled out */
  if((hop = trace_hop(pr, dl->dl_af, dl->dl_ip_src, trace)) == NULL)
    goto err;

  /* for HTTP payload print */
  if (dl->dl_tcp_datalen > 0)
  {
    hop->hop_tcp_payload = 1;
    
    trace->payload = realloc(trace->payload, dl->dl_tcp_datalen + trace->bytes_copied);
    memcpy(trace->payload + trace->bytes_copied, dl->dl_tcp_data, dl->dl_tcp_datalen); 
    trace->bytes_copied += dl->dl_tcp_datalen;
  }
  else
  {
    hop->hop_tcp_payload = 0;
  }
			    ...
			    trace_probe_t *probe)
{
  ...
  //   (trace->server_try == 1 && probe->ttl != SCAMPER_DO_TRACE_HTTP_TTL_DEF))
  // {
  //   return 0;
  // }
  ...
  trace_hopins(&trace->hops[hop->hop_probe_ttl-1], hop, trace);

  /* If we receive an icmp reply for our syn packet */
  if (probe->ttl == SCAMPER_DO_TRACE_HTTP_TTL_DEF) 
   {
   	  trace->hop_count++;
  	  state->ttl++; 
   ...
   else if (trace->http_string != NULL && 
    (trace->server_try == 1 && probe->ttl != SCAMPER_DO_TRACE_HTTP_TTL_DEF))
   {
   	  return 0;
   }
				...
				trace_probe_t *probe)
{
  ...
  if(probe->mode == MODE_TRACE)
      ...
      if((hop = trace_icmp_hop(trace, probe, ir)) == NULL)
	{
      ...
      trace_hopins(&trace->lastditch, hop, trace);
 ...
 if((probe->ttl != SCAMPER_DO_TRACE_HTTP_TTL_DEF))
  {
	  ...
	  {
		  	if((trace->confidence == 0 && trace->http_retries >= SCAMPER_DO_TRACE_HTTP_RTRY_DEF 
		  		&& trace->server_try == 0) || (trace->confidence != 0 && state->attempt == k(state)))
		    {
		      /* we're probably going to send another probe, so reset the attempt # */
		      state->attempt = 0;
			  ...
			  trace_probe_t *probe)
{
  ...
  if((hop = trace_dl_hop(probe, dl, trace)) == NULL)
    return -1;
  trace_hopins(&trace->hops[hop->hop_probe_ttl-1], hop, trace);
...
static void do_trace_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl)
{
  ...
  if(direction == 0)
  {
    ...
    */
      if((probe->flags & TRACE_PROBE_FLAG_DL_RX) != 0) // syn flag check
      {
      ...
      if(handletp_func[probe->mode] != NULL)
      {
        ...
        if(trace->http_string == NULL)
        {
      	...
      	/* valid syn ack reply from the server */
        if (probe->ttl == SCAMPER_DO_TRACE_HTTP_TTL_DEF && 
           (dl->dl_tcp_flags & (TH_SYN | TH_ACK))  == (TH_SYN | TH_ACK))
        {
          trace->ack_num = dl->dl_tcp_seq;
          trace->server_try  = 0;
          trace_queue(task);
          return;
        }
        /* not valid syn ack retransmissions - ignore them */
        else if ((dl->dl_tcp_flags & (TH_SYN | TH_ACK))  == (TH_SYN | TH_ACK))
        { // tcp syn retrans after icmp resp
          trace_queue(task);
          return;
        }
...
static void do_trace_write(scamper_file_t *sf, scamper_task_t *task)
{
  ...
  if(trace->http_string != NULL && trace->http_file && trace->payload != NULL)
  {
  	  size_t wc;
  	  ...
  	  snprintf(path, sizeof(path), "custom-files/http_payload/%s.txt",
  ...
  scamper_file_write_trace(sf, trace);
  return;
}
...
static void do_trace_probe(scamper_task_t *task)
{
  ...
  /* stop conditions */
  if(trace->http_string != NULL && (trace->server_reached_flag || 
    ((trace->syn_retries >= SCAMPER_DO_TRACE_SYN_RTRY_DEF) && trace->server_try)||
    trace->tp_ttl > SCAMPER_DO_TRACE_HOPCOUNT_DEF))
    {
  	  trace_stop_completed(trace);
  	  scamper_task_queue_done(task, 0);
	    return;
    ...
    if(trace->server_try)
     {
        ...
        probe.pr_ip_ttl    	 = SCAMPER_DO_TRACE_HTTP_TTL_DEF;
        probe.pr_tcp_seq   	 = 0;
        probe.pr_tcp_ack   	 = 0;
        probe.pr_tcp_win   	 = 0;

    	trace->wait_probe  	 = (uint8_t)0;
    	trace->wait       	 = (uint8_t)10;

        trace->http_retries  = 0;
        trace->syn_retries++;
     }
    else
     {

    	probe.pr_data      = trace->http_string;
        probe.pr_len       = strlen(probe.pr_data);
        probe.pr_tcp_flags = TH_PUSH | TH_ACK;

        probe.pr_ip_ttl    = trace->tp_ttl;
        probe.pr_tcp_seq   = 1;
        probe.pr_tcp_ack   = trace->ack_num + 1;
        probe.pr_tcp_win   = SCAMPER_DO_TRACE_HTTP_WIN_DEF;

        trace->wait_probe  = (uint8_t)200;
        trace->wait        = (uint8_t)2;
  ...
  /* note another probe sent */
  trace->probec++;

  timeval_cpy(&tp->tx_tv, &probe.pr_tx); // Time we sent probe
  ...
  /* do not append when retransmission */
  if ((trace->syn_retries == 1 && trace->http_retries == 0) ||
      (trace->syn_retries == 0 && trace->http_retries >= 1) ||
      (trace->http_string == NULL))
  {
    state->id_next++; 
    state->attempt++;
  }
  ...
  /* add previous time and waitprobe time */
  if(trace->wait_probe > 0)
...
void *scamper_do_trace_alloc(char *str)
{
  ...
  trace->offset      		     = offset;
  trace->userid      		     = userid;
  trace->http_string         	 = http_string; http_string = NULL;
  trace->tp_ttl              	 = firsthop;
  trace->http_file 				 = http_file;
  trace->syn_retries         	 = 0;
  trace->http_retries        	 = 0;
  trace->server_reached_flag 	 = 0;
  trace->server_try			     = 1;
  trace->bytes_copied			 = 0;
 ...
 err:
  if(http_string != NULL) free(http_string);

...
def dns_resolver_3(hostname, rdtype, cnames, name_servers,name_servers_answered,response_code):
			...
			try:
					...
					if len(response.additional) > 0:   # use the IP in ADDITIONAL section
						for rrset in response.additional:
							...
							try:
								...
								print("This is what I am pushing \n",(" ".join(str(response2).split("\n"))))
								response_code.append((" ".join(str(response2).split("\n"))))
								print("This is what I received\n",str(response2).split("\n"))
								if len(response2.authority)>0:
									print("authority section \n",(response2.authority[0]))
									if "SOA" in str(response2.authority[0]):
										print("''''''''''''''''''''''''''''''''''''''''''")
										stop_flag=True
										#print(dir(response2.rcode))
								#		print(response2.rcode)
										name_servers_answered.append((hostname,str(response2.authority[0]),"Could not get IP"))
										print("Jinagala True True \n",str(response2).split(" ")[3])
										return ""
						...
						if check_hostname(ns):
							...
							response2 = dns_resolver_3(ns, 'A', cnames,name_servers,name_servers_answered,response_code)
							if stop_flag:
								return ""
				...
				#    print(response)
					...
					for rrset in response.answer:
						...
						return dns_resolver_3(cname, rdtype, cnames,name_servers,name_servers_answered,response_code)
				break
			except Exception as e:
				if str(e)=="Timeout":
					return ""
...
def alias(array):
	if len(array) == 3:
		...
		signal.signal(signal.SIGALRM, handler)
		signal.alarm(30)
		myresponse = dns_resolver_3(hostname, rdtype, cnames,name_servers,name_servers_answered,response_code)
		signal.alarm(0)
		...
		if length!=0 and (name_servers_answered[length-1][2]=="IP") and myresponse:
			...
			with open("name_server_ip",'a') as file1:
			...
			for one_name_server in name_servers_answered:
				...
				if one_name_server[2]=="Could not get IP" and one_name_server[0]==real_host_domain:
					print("HELLO")
					real_host=real_host+" "+one_name_server[1]
			if len(response_code)>0:
				message=""
				message=real_host_domain+" "+str(response_code[len(response_code)-1])+"\n"
				with open("las_response_received.txt",'a') as file10:
			...
			with open("name_server_ip_fail",'a') as file1:

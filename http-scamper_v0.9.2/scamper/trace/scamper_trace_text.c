  ...
  if((hop->hop_flags & SCAMPER_TRACE_HOP_FLAG_TCP) != 0)
    {
      if((hop->hop_tcp_flags & TH_RST) != 0)
    	{
    	  snprintf(str, len, " [TCP, RST]");
    	}
      else if((hop->hop_tcp_flags & (TH_SYN|TH_ACK)) == (TH_SYN|TH_ACK))
  	{
  	  if((hop->hop_tcp_flags & TH_ECE) != 0)
  	    snprintf(str, len, " [SYN-ACK, ECN]");
  	  else
  	    snprintf(str, len, " [SYN-ACK]");
  	}
        else
  	{
	  if(hop->hop_tcp_flags == 0)
	    {snprintf(str, len, " [unknown, no flags]");}
	  else if(hop->hop_tcp_payload)
	    {snprintf(str, len, " [TCP-HTTP,%s%s%s%s%s%s%s%s]",
      ...
      {snprintf(str, len, " [TCP,%s%s%s%s%s%s%s%s]",
         (hop->hop_tcp_flags & TH_RST)  ? " RST" : "",
         (hop->hop_tcp_flags & TH_SYN)  ? " SYN" : "",
         (hop->hop_tcp_flags & TH_ACK)  ? " ACK" : "",
         (hop->hop_tcp_flags & TH_PUSH) ? " PSH" : "",
  ...
  if(trace->src != NULL)
    {
      ...
      if(SCAMPER_TRACE_TYPE_IS_ICMP(trace))
        {snprintf(header, sizeof(header), "icmp-traceroute from %s to %s", src, dst);}
      else if(SCAMPER_TRACE_TYPE_IS_UDP(trace) || SCAMPER_TRACE_TYPE_IS_UDP_PARIS(trace))
        {snprintf(header, sizeof(header), "udp-traceroute from %s to %s", src, dst);}
        ...
        {snprintf(header, sizeof(header), "http-traceroute from %s to %s", src, dst);}
    }
  else
    {
